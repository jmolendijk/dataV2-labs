...
def calc():  
    ...
    y = input('What do you want to do? plus or minus: ')
    z = input('Please choose your second number (zero to five): ')
    try:
        res = operations[y.lower()](numbers[x.lower()], numbers[z.lower()])
        if res < 0:
            res = "minus "+results[abs(res)]
        else:
            res = results[abs(res)]
    except:
        print("I am not able to answer this question. Check your input.")
    else:
        return print(f'{x.lower().capitalize()} {y} {z.lower().capitalize()} equals {res}')
